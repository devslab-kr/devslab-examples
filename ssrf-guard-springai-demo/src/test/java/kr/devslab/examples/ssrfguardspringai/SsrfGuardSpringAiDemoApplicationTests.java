 ...
 *   FakeLlmService → wrapped ToolCallback → guard reject / approve.
 *
 * <p>None of these touch the network — the underlying tool is a pretend
 * fetch. The point is to assert that the guard fires (or doesn't) on the
 * tool *input* before the tool body would have made any HTTP call.
 */
@SpringBootTest
@AutoConfigureMockMvc
class SsrfGuardSpringAiDemoApplicationTests {

    @Autowired
    private MockMvc mockMvc;

    @Test
    void legitimateUrlIsAllowedThroughToTheTool() throws Exception {
        mockMvc.perform(post("/agent/chat")
                        .param("message", "Please fetch https://httpbin.org/get for me"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.blocked").value(false))
                // The tool "pretend-fetches" — that string is its signal
    ...
    @Test
    void promptWithoutUrlDoesNotInvokeTheTool() throws Exception {
        mockMvc.perform(post("/agent/chat")
                        .param("message", "Just say hi — no URL in here"))
                .andExpect(status().isOk())
                // The fake LLM short-circuits and reports "no tool call" —
                // the wrap never sees an input because no input was constructed.
