 ...
 *       URL reaches the {@code ToolCallback.call()} entry point, ssrf-guard
 *       behaves identically whether a human, a fake LLM, or GPT-5 supplied
 *       the URL.</li>
 *   <li>Determinism — tests can assert exactly which tool got invoked with
 *       which arguments.</li>
 * </ul>
 *
 * <p>Swap this class for a real {@code ChatClient} (Spring AI 1.0+) with the
 * same {@code ToolCallback} array and the demo's behaviour stays correct.
 */
@Service
public class FakeLlmService {
    ...
    private final ToolCallback fetchUrlTool;

    public FakeLlmService(ToolCallback fetchUrlTool) {
        // Spring injects the ssrf-guard-WRAPPED instance here — not the raw
        // one defined in FetchUrlTool. The BeanPostProcessor that does the
        // wrapping runs before any dependency injection, so by the time the
        // service constructor fires there's only one ToolCallback in the
        // context and it's already secured.
        this.fetchUrlTool = fetchUrlTool;
    }
    ...
    public Map<String, Object> chat(String userMessage) {
        ...
        // What a real LLM emits to the tool dispatcher: a JSON blob naming
        // the tool and its arguments. We construct it by hand to keep the
        // demo legible.
        String toolInput = "{\"url\":\"" + url.replace("\"", "\\\"") + "\"}";
        trace.put("toolCall", Map.of(
                "name", fetchUrlTool.getToolDefinition().name(),
                "input", toolInput
        ));

        // THIS is the call ssrf-guard-springai intercepts. If the URL is
        // disallowed, the wrap returns a JSON error string (the LLM-friendly
        // path); we never reach the tool's pretend-fetch logic.
        String toolOutput = fetchUrlTool.call(toolInput);
        ...
        // that's the contract documented in SsrfGuardedToolCallback.
    ...
    public static java.util.List<String> attackScenarios() {
        return Arrays.asList(
                // The same 12 scenarios from the basic demo, phrased as
                // natural-language prompts an attacker might send to an
                // LLM-powered agent.
