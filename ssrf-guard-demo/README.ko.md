...
- `RestClient` (Spring 6.1+) — 메타 아티팩트 `kr.devslab:ssrf-guard:3.2.0`
- `RestTemplate` — `kr.devslab:ssrf-guard-resttemplate:3.2.0`
- `WebClient` (WebFlux) — `kr.devslab:ssrf-guard-webclient:3.2.0`
...
| 파일 | 왜 |
| --- | --- |
| `build.gradle.kts` | 표준 스타터 외 의존성은 `kr.devslab:ssrf-guard:3.2.0`, `:ssrf-guard-resttemplate:3.2.0`, `:ssrf-guard-webclient:3.2.0` 셋뿐 — 별도 configuration 클래스 불필요 |
