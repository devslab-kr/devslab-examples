...
- `RestClient` (Spring 6.1+) via the meta `kr.devslab:ssrf-guard:3.2.0` artifact
- `RestTemplate` via `kr.devslab:ssrf-guard-resttemplate:3.2.0`
- `WebClient` (WebFlux) via `kr.devslab:ssrf-guard-webclient:3.2.0`
...
| File | Why |
| --- | --- |
| `build.gradle.kts` | The only dependencies beyond the standard starters are `kr.devslab:ssrf-guard:3.2.0`, `kr.devslab:ssrf-guard-resttemplate:3.2.0`, `kr.devslab:ssrf-guard-webclient:3.2.0` — no manual configuration class needed |
